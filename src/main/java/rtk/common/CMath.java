...
public class CMath {
    ...
    public static Vector3d randomVector(double length) {
        return randomVector().normalize().scale(length);
    }

    public static Vector3d randomVector(double length, double angleSpread, Vector3d heading) {
        if (Math.abs(angleSpread) < Math.PI / 180)
            return heading;

        double minDot = Math.cos(angleSpread) * length * heading.length();
        for (int i = 0; i < 100; i++) {
            Vector3d result = randomVector(length);
            if (result.dotProduct(heading) >= minDot)
                return result;
        }

        return heading; // If we didn't hit it after 50 tries the target must be small so using the
                        // heading is fair.
    }

    public static Vector3d randomVector(Vector3d heading, double length, double inaccuracy) {
        return heading.normalize().add(randomVector().scale(0.007499999832361937D * inaccuracy)).normalize()
                .scale(length);
    }
